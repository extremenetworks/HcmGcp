...
public class MetricsRes {
	...
	/** Retrieves all resources (VMs, subnets, networks, etc.) for the given project ID from the DB */
    @GET
    @Path("all")
    public String retrieveAllmetrics(
    ...
    /** Starts a background worker that pulls all resources from the given account. This is a non-blocking REST call
     * that just starts that worker in a separate thread and immediately responds to the caller. Once the background
     * worker is done retrieving all data from AWS it will
     * 	- update the DB
    ...
    public String triggerUpdateAll(
    	...
    	try {
    		...
    		executor.execute(new MetricsWorker(
    				projectId, authFileContent, RABBIT_QUEUE_NAME, rabbitChannel));
            
            return jsonMapper.writeValueAsString(
            		new ResourcesWebResponse(0, "Successfully triggered an update of all metrics data"));
            
    	} catch (Exception ex) {
    		logger.error("Error parsing parameters and trying to setup the background worker to trigger an update on all metrics data", ex);
    		return "";
    	}
    }
    
    
    /** Retrieves all resource data for the given account from the DB. Generate a JSON-formated string. Example:
     * {
	    "dataType": "resources",
	    ...
	    "data": [
	        {
	            "lastUpdated": "2019-04-05 15:22:38",
	            "resourceType": "Subnet",
	            "resourceData": [
	                {
	                    "tags": [],
    ...
    private String retrieveDataFromDb(String projectId) {
        ...
        try {
            ...
            while (rs.next()) {
            	...
            	logger.debug("Retrieved next DB row: " 
            		+ ", lastUpdated: " + rs.getString("lastUpdated")
            		+ ", resourceType: " + rs.getString("resourceType")
            		+ ", resourceData: " + rs.getString("resourceData").substring(0, 200) + "...");
